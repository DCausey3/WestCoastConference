...
// The three districts are static pages (app/districts/{slug}/page.tsx). We key
// off the structured `slug` field rather than matching the free-text `name`
// string — name-matching silently dropped any district whose Sanity name
// didn't exactly match a hardcoded string, with zero visible error.
const KNOWN_SLUGS = ['lakeland', 'stpete', 'tampa'];
...
export default function DistrictsLanding({ districts: sanityDistricts }: DistrictsLandingProps) {
    const districts = sanityDistricts?.length
        ? sanityDistricts
            .filter((d) => d.slug?.current && KNOWN_SLUGS.includes(d.slug.current))
            .map((d) => {
                const id = d.slug!.current;
    ...
    return (
        <div className="bg-white">
            {/* FULL-BLEED HERO */}
            <section className="relative" style={{ minHeight: '640px' }}>
                <div
                    className="absolute inset-0 bg-cover bg-center"
                    style={{ backgroundImage: 'url(/assets/districts/districts-collage-hero.png)' }}
                />
                <div
                    className="absolute inset-0"
                    style={{ background: 'linear-gradient(180deg, rgba(10,31,68,0.35) 0%, rgba(10,31,68,0.55) 55%, rgba(10,31,68,0.92) 100%)' }}
                />

                <div className="relative max-w-5xl mx-auto px-6 flex flex-col items-center justify-end text-center" style={{ minHeight: '640px', paddingBottom: '4rem' }}>
                    <div
                        className="text-[#C9A84C] mb-4"
                        style={{ fontFamily: "'Source Sans 3', sans-serif", fontWeight: 600, fontSize: '13px', letterSpacing: '0.2em' }}
                    >
                        WEST COAST CONFERENCE · AME CHURCH
                    </div>
                    <h1 className="text-white mb-6" style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(2.75rem, 6vw, 4rem)', fontWeight: 700, lineHeight: '1.1' }}>
                        Three Districts, One Family
                    </h1>
                    <div className="h-1 w-24 bg-[#C9A84C] mb-6"></div>
                    <p className="text-white/85 max-w-2xl" style={{ fontSize: '18px', lineHeight: '1.7' }}>
            ...
            {/* DISTRICT CARDS */}
            <section className="py-20 px-6 bg-gradient-to-b from-white to-[#F4F6FA]">
